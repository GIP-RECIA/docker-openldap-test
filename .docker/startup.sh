...
WAS_INIT_DB_SET="/etc/ldap/slapd.d/docker-openldap-was-init-db-set"
...
if [ ! -e "$FIRST_START_DONE" ]; then
  ...
  function ldap_add_or_modify (){
    ...
    if grep -iq changetype $LDIF_FILE ; then
        ( ldapmodify -Y EXTERNAL -Q -H ldapi:/// -f $LDIF_FILE 2>&1 || ldapmodify -h localhost -p 389 -D cn=admin,ou=administrateurs,$LDAP_BASE_DN -w "$LDAP_ADMIN_PASSWORD" -f $LDIF_FILE 2>&1 ) | log-helper debug
    else
        ( ldapadd -Y EXTERNAL -Q -H ldapi:/// -f $LDIF_FILE 2>&1 || ldapadd -h localhost -p 389 -D cn=admin,ou=administrateurs,$LDAP_BASE_DN -w "$LDAP_ADMIN_PASSWORD" -f $LDIF_FILE 2>&1 ) | log-helper debug
    fi
  }
...
EOF
  ...
  if [ "${KEEP_EXISTING_CONFIG,,}" == "true" ]; then
    ...
    if [[ -f "$WAS_INIT_DB_SET" ]]; then
      for f in $(find ${CONTAINER_SERVICE_DIR}/slapd/assets/init-db -type f -name \*.ldif  | sort); do
        ldap_add_or_modify "$f"
      done
    else
       touch "$WAS_INIT_DB_SET"
    fi
